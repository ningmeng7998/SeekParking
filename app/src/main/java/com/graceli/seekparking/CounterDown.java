    ...
    void startTimer() {
        cTimer = new CountDownTimer( LastTimeMin_int * 60000, 1000) {
            public void onTick(long millisUntilFinished) {
                ...
                if(noti_1==0)
                {
                    if(l_Time_Min<NotiTimeMin_int)
                    {
                        ...
                        noti_1=1;
                        noti_2=0;
                        ShowNoti("Seek Parking","Parking Time remaining : " + String.valueOf(NotiTimeMin_int) + " Min",1);
                    }
                }
            }
            public void onFinish() {
                ...
                txt_Timer_Sec.setText("00");
                btn_Cancel.setText("Back to Seek Parking");
                lyTC.setBackgroundColor(Color.parseColor("#FF6E6E"));
                if(noti_2==0)
                {
                    noti_2 = 1;
                    ShowNoti("Seek Parking","Your parking time is over",2);
                }
            }
        };
        cTimer.start();
